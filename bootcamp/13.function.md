...
### Inline Functions
An inline function is a function that is expanded in line when it is invoked, meaning the compiler attempts to insert the function’s code at the point of each call, instead of performing a regular function call. 
...
int main() {
    int result = multiply(3, 4); // The compiler replaces multiply(3, 4) with its code, avoiding a function call.
...
```
**multiply** function:
*   **keyword**: `inline` before the function’s return type.
*   **returnType**: `int`
...
> [!WARNING]
> Not all functions are suitable for inlining. Typically, large or complex functions, as well as those that involve recursion, are not inlined by the compiler.
